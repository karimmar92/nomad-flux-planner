 ...
 *
 * `founding_spots_taken` and `claim_founding_spot` are live and present in the
 * generated Supabase types, so these are plain typed rpc calls. The wrappers
 * remain because the error handling below is a decision, not boilerplate: see
 * each function.
 */
 ...
 */
export async function fetchFoundingTaken(client: Client): Promise<number | null> {
